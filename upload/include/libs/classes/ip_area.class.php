 ...
 *
 * @copyright			(C) 2008-2014 KuaiFanCMS
 * @license				http://www.kuaifan.net/license/
 */
class ip_area {
	...
	public function __construct() {
		if(@file_exists(KF_INC_PATH.DIRECTORY_SEPARATOR.'libs'.DIRECTORY_SEPARATOR.'data'.DIRECTORY_SEPARATOR.'ipdata'.DIRECTORY_SEPARATOR.'mini.Dat')) {
			$this->func = 'data_mini';
			$this->fp = @fopen(KF_INC_PATH.DIRECTORY_SEPARATOR.'libs'.DIRECTORY_SEPARATOR.'data'.DIRECTORY_SEPARATOR.'ipdata'.DIRECTORY_SEPARATOR.'mini.Dat', 'rb');
			$this->offset = unpack('Nlen', fread($this->fp, 4));
			$this->index  = fread($this->fp, $this->offset['len'] - 4);
		} elseif(@file_existes(KF_INC_PATH.DIRECTORY_SEPARATOR.'libs'.DIRECTORY_SEPARATOR.'data'.DIRECTORY_SEPARATOR.'ipdata'.DIRECTORY_SEPARATOR.'QQWry.Dat')) {
			$this->func = 'data_full';
			$this->fp = @fopen(KF_INC_PATH.DIRECTORY_SEPARATOR.'libs'.DIRECTORY_SEPARATOR.'data'.DIRECTORY_SEPARATOR.'ipdata'.DIRECTORY_SEPARATOR.'QQWry.Dat', 'rb');
		}
	}
	...
	*/
	public function getcitybyapi($ip) {
		$api_url = $localinfo = '';
		$xml = kf_class::run_sys_class('xml');
		kf_class::run_sys_func('iconv');
		...
		if($data['code'] == 0) {
			$localinfo['province'] = $data['data']['country'];
			$localinfo['city'] = $data['data']['city'];
			$localinfo['sp'] = $data['data']['isp'];		
			//$name = strtolower(CHARSET) == 'gbk' ? $localinfo['city'] : iconv(CHARSET,'gbk',$localinfo['city']);
			$name = str_replace('市','',$localinfo['city']);
