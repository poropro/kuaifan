...
class segment {
	...
	function __construct($loaddic=true) {
	  ...
	  @ini_set('memory_limit','128M');
  	  ...
  	  $dicfile = KF_ROOT_PATH.'include'.DIRECTORY_SEPARATOR.'libs'.DIRECTORY_SEPARATOR.'data'.DIRECTORY_SEPARATOR.'dict'.DIRECTORY_SEPARATOR.'dict.csv';
  	  $fp = fopen($dicfile,'r');
  ...
  function get_source($str) {
  	if(CHARSET == 'utf8') $str = iconv('utf-8','gbk',$str);
  ...
  function split_result($str='',$try_num_name=true,$try_diff=true) {
  	...
  	if(CHARSET=='utf8') $okstr = iconv('gbk','utf-8',$this->result_string);
  ...
  function get_keyword($str,$ilen=-1) {
    ...
    if(CHARSET=='utf8') $okstr = iconv('gbk','utf-8',$okstr);
    return trim($okstr);
  }
  function get_rank($w){
