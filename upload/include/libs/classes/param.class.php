...
class param {
	...
	public function __construct() {
		...
		$this->route_config = kf_class::run_config('route', SITE_URL) ? kf_class::run_config('route', SITE_URL) : kf_class::run_config('route', 'default');
	...
	public static function set_cookie($var, $value = '', $time = 0) {
		...
		$var = $_CFG['cookie_pre'].$var;
		$_COOKIE[$var] = $value;
		if (is_array($value)) {
			foreach($value as $k=>$v) {
				setcookie($var.'['.$k.']', sys_auth($v, 'ENCODE'), $time, $_CFG['cookie_path'], $_CFG['cookie_domain'], $s);
			}
		} else {
			setcookie($var, sys_auth($value, 'ENCODE'), $time, $_CFG['cookie_path'], $_CFG['cookie_domain'], $s);
		}
	}
