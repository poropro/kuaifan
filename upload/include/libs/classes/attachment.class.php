...
class attachment {
	...
	function __construct($module='', $catid = 0,$siteid = 0,$upload_dir = '') {
		...
		$this->upload_root = kf_class::run_config('system','upload_path');
	...
	{
		global $image_d;
		$this->att_db = kf_class::run_model('attachment_model');
		$upload_url = kf_class::run_config('system','upload_url');
		$this->field = $field;
		$dir = date('Y/md/');
	...
	function add($uploadedfile) {
		...
		$uploadedfile['status'] = kf_class::run_config('system','attachment_stat') ? 0 : 1;
	...
	*/
	...
	function uploaderror($id = 0)	{
		file_put_contents(KF_ROOT_PATH.'xxx.txt', $id);
	}
