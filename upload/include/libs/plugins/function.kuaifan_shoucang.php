<?php
function smarty_function_kuaifan_shoucang($params, &$smarty)
{
	global $db;
	...
	$orderbysql=" ORDER BY adddate DESC,id DESC";
	$wheresql = '';
	...
	if (isset($aset['paged']))
	{
		...
		$total_sql="SELECT COUNT(*) AS num FROM ".table('shoucang').$wheresql;
	...
	$result = $db->query("SELECT * FROM ".table('shoucang')." ".$wheresql.$orderbysql.$limit);
	$list= array();
	$__n= 1;$_timearrid = '';
	while($row = $db->fetch_array($result))
	{
		...
		$row['url_']=$row['url'];
		$row['url']=str_replace("&","&amp;", $row['url'])."&amp;sid={$_GET['sid']}&amp;vs={$_GET['vs']}";
		$__n ++;
		$list[] = $row;
	}
