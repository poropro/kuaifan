<?php
function smarty_function_kuaifan_xinxi_xitong($params, &$smarty)
{
	global $db;
	...
	if ($aset['status']=='1'){
		$wheresql.= "AND id not in (SELECT group_message_id FROM ".table('xinxi_data')." WHERE userid={$aset['userid']})";
	}
	if ($aset['status']=='0'){
		$wheresql.= "AND id in (SELECT group_message_id FROM ".table('xinxi_data')." WHERE userid={$aset['userid']})";
	}
	...
	if (isset($aset['paged']))
	{
		...
		$total_sql="SELECT COUNT(*) AS num FROM ".table('xinxi_xitong').$wheresql;
	...
	$result = $db->query("SELECT * FROM ".table('xinxi_xitong')." ".$wheresql.$orderbysql.$limit);
	$list= array();
	$__n= 1;
	while($row = $db->fetch_array($result))
	{
		$row['_n']=$__n+($currenpage*$aset['row'])-$aset['row'];
		$row['status'] = $db->get_total("SELECT COUNT(*) AS num FROM ".table('xinxi_data')." WHERE userid={$aset['userid']} AND group_message_id=".$row['id']);
