<?php
function smarty_function_kuaifan_neirong_fujian($params, &$smarty)
{
	global $db;
	...
	$orderbysql=" ORDER BY `addtime` DESC ,id DESC";
	if (!empty($aset['keyname'])){
		$keyname=trim($aset['keyname']);
		$wheresql.=" AND (`title` like '%{$keyname}%' or `name` like '%{$keyname}%')";
	...
	if (isset($aset['paged']))
	{
		...
		$total_sql="SELECT COUNT(*) AS num FROM ".table('neirong_fujian').$wheresql;
	...
	$result = $db->query("SELECT * FROM ".table('neirong_fujian')." ".$wheresql.$orderbysql.$limit);
	$list= array();
	$__n= 1;
	while($row = $db->fetch_array($result))
	{
		...
		$row['name_']=$row['name'];
		$row['name']=cut_str($row['name'],$aset['titlelen'],0,$aset['dot']);
		$__n ++;
		$list[] = $row;
	}
