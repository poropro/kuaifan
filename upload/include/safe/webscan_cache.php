<?php
//用户唯一key
define('WEBSCAN_U_KEY', '8cb9bf5c59dddd808ef7269524b9c1ee');
//数据回调统计地址
define('WEBSCAN_API_LOG', 'http://safe.webscan.360.cn/papi/log/?key='.WEBSCAN_U_KEY);
//版本更新地址
define('WEBSCAN_UPDATE_FILE','http://safe.webscan.360.cn/papi/update/?key='.WEBSCAN_U_KEY);
//拦截开关(1为开启，0关闭)
$webscan_switch=1;
//提交方式拦截(1开启拦截,0关闭拦截,post,get,cookie,referre选择需要拦截的方式)
$webscan_post=1;
$webscan_get=1;
$webscan_cookie=1;
$webscan_referre=1;
//后台白名单,后台操作将不会拦截,添加"|"隔开白名单目录下面默认是网址带 admin 放行
$webscan_white_directory='';
//url白名单,可以自定义添加url白名单,默认是对kuaifan的后台url放行
//写法：比如kuaifan 后台操作url index.php?m=admin
$webscan_white_url = array();
if($_GET['m']=='admin' && $_GET['c']!='login'){
	$webscan_white_url = array('index.php' => 'm=admin');
}
?>