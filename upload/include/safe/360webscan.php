...
function webscan_StopAttack($StrFiltKey,$StrFiltValue,$ArrFiltReq,$method) {
  ...
  if (preg_match("/".$ArrFiltReq."/is",$StrFiltValue)==1){
    ...
    $_CFG['open_sql_true'] = '1'; //开源行
	if ($method=='GET') unset($_GET[$StrFiltKey]);
	if ($method=='POST') unset($_POST[$StrFiltKey]);
	if ($method=='COOKIE') unset($_COOKIE[$StrFiltKey]);
	if ($method=='REFERRER') unset($webscan_referer[$StrFiltKey]);
    //exit(webscan_pape());
  }
