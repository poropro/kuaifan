...
<a href="{#kuaifan getlink='a'#}&amp;a=info">返回</a><br/>
{#form set="头" notvs="1"#}
项目名称:(仅作为后台记录)<br/>{#form set="输入框|名称:'title{#$TIME2#}'" data_value=$show.title htmlspecialchars="1"#}<br/>
-------------<br/>

{#include file="admin/paiban/type/{#$show.type_en#}.tpl" value=$show.body#}
-------------<br/>

内容前面:(支持ubb,如:[br]自动换行)<br/>{#form set="输入框|名称:'qianmian{#$TIME2#}',最多字符:300" data_value=$show.qianmian#}<br/>
内容后面:(支持ubb,如:[br]自动换行)<br/>{#form set="输入框|名称:'houmian{#$TIME2#}',最多字符:300" data_value=$show.houmian#}<br/>
项目排序:(越大越靠前)<br/>{#form set="输入框|名称:'order{#$TIME2#}'" data_value=$show.order#}<br/>
显示版本:(<a href="{#get_link('vs','',1)#}&amp;m=explain&amp;l=paiban&amp;a=wap&amp;go_url={#urlencode(get_link('','&'))#}">说明</a>)<br/>
{#form set="输入框|名称:'wap{#$TIME2#}',最多字符:100,值:0" data_value=$show.wap#}<br/>
登录可见:{#form set="列表框|名称:'islogin{#$TIME2#}'" list="默认:0,仅登录:1,仅游客:2" default=$show.islogin#}<br/>
是否显示:{#form set="列表框|名称:'hide{#$TIME2#}'" list="是:0,否:1" default=$show.hide#}<br/>
缓存时长:{#form set="输入框|名称:'nocache{#$TIME2#}',宽:5" data_value=$show.nocache#}分(<a href="{#get_link('vs','',1)#}&amp;m=explain&amp;l=paiban&amp;a=nocache&amp;go_url={#urlencode(get_link('','&'))#}">说明</a>)<br/>
